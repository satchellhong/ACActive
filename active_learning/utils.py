...
def smiles_to_murcko_ecfp(smiles: list[str], radius: int = 2, nbits: int = 1024, silent: bool = True, to_array: bool = True) \
        -> np.ndarray:
    """ Get a Numpy array of ECFPs from a list of SMILES strings """
    from rdkit.Chem.AllChem import GetMorganFingerprintAsBitVect
    from rdkit.Chem import MolFromSmiles
    ...
    scaffold = [Chem.MolToSmiles(MurckoScaffold.GetScaffoldForMol(Chem.MolFromSmiles(s))) for s in smiles]
    fp = [GetMorganFingerprintAsBitVect(MolFromSmiles(s), radius, nBits=nbits) for s in tqdm(scaffold, disable=silent)]
...
def bedroc_score(y_true, y_pred, alpha = 20):
    """
...
class Evaluate:
    ...
    def eval(self, logits_N_K_C: torch.Tensor, y: torch.Tensor):
        ...
        # print(logits_N_K_C[0])

        if screen2:
            y_hat = torch.mean(logits_N_K_C, dim=1)
        ...
        try:
            ...
            bedroc = bedroc_score(y, y_hat, alpha = 80.5)
            bedauc20 = bedroc_score(y, y_hat, alpha = 20)
            self.bedauc.append(bedroc)
            self.bedauc20.append(bedauc20)
        except:
            self.roc_auc.append(0)
            self.bedauc.append(0)
            self.bedauc20.append(0)
    ...
    def eval_2(self, logits_N_K_C: torch.Tensor, y: torch.Tensor):
        screen2 = True
        y = y.cpu() if type(y) is torch.Tensor else torch.tensor(y)
        # print(logits_N_K_C[0])
        ...
        try:
            ...
            self.roc_auc.append(0)
        try:
            bedauc = bedroc_score(y, y_hat, alpha = 80.5)
            self.bedauc.append(bedauc)
        except:
            self.bedauc.append(0)
        try:
            bedauc20 = bedroc_score(y, y_hat, alpha = 20)
    ...
    def eval2(self, logits_N_K_C: torch.Tensor, y: torch.Tensor):
    ...
    def __repr__(self):
        return f"Binary accuracy:    {self.binary_accuracy[-1]:.4f}\n" \
               ...
               f"BED AUC:            {self.bedauc[-1]:.4f}\n" \
               f"BED AUC20:          {self.bedauc20[-1]:.4f}\n" \
    ...
    def to_dataframe(self, colnames: str = ''):
        # print(len(self.ef))
        # print(len(self.balanced_accuracy))
        # print(len(self.bedauc))
        # print(len(self.bedauc20))
        # print(len(self.tp))
        # print(len(self.binary_accuracy))
        # print(len(self.roc_auc))
        # print(len(self.fp))
        # print(len(self.tn))
        # print(len(self.fn))
        # print(len(self.precision))
        # print(len(self.tpr))
        df = pd.DataFrame({'cycle': list(range(len(self.tp))), 'binary_accuracy': self.binary_accuracy,
                           'balanced_accuracy': self.balanced_accuracy, 'roc_auc': self.roc_auc, 'bed_auc': self.bedauc, 'bed_auc20': self.bedauc20, 
                           'EF0.5': [x[0] for x in self.ef], 'EF1': [x[1] for x in self.ef], 'EF5': [x[2] for x in self.ef],
    ...
    def to_dataframe2(self, top, colnames: str = ''):
        df = pd.DataFrame({'cycle': list(range(len(self.tp))), 'binary_accuracy': self.binary_accuracy,
                           'balanced_accuracy': self.balanced_accuracy, 'roc_auc': self.roc_auc, 'bed_auc': self.bedauc, 'bed_auc20': self.bedauc20, 
                           'EF0.5': [x[0] for x in self.ef], 'EF1': [x[1] for x in self.ef], 'EF5': [x[2] for x in self.ef],
                           'precision': self.precision, 'tpr': self.tpr,
...
def to_torch_dataloader_multi(xs, x: Union[list, np.ndarray], y: Optional[np.ndarray] = None, pred_list = None, **kwargs) -> \
    ...
    if type(x) is np.ndarray:
        assert y is not None, 'No y values provided'
        x = [torch.tensor(arr, dtype=torch.float32) if isinstance(arr, np.ndarray) else arr
                for arr in xs]
        if pred_list is not None:
            seqs = [torch.stack(preds).unsqueeze(-1).float() for preds in pred_list]
        ...
        if len(xs) > 0:
            for i, data in enumerate(x):
                fp_list = [torch.tensor(xs[j][i], dtype=torch.float32) for j in range(len(xs))]
                data.fp = torch.stack(fp_list, dim=0).unsqueeze(0)
        return pyg_DataLoader(x, **kwargs)

def to_weighted_torch_dataloader(x: Union[list, np.ndarray], y: Optional[np.ndarray] = None, w = None, **kwargs) -> \
        Union[DataLoader, pyg_DataLoader]:

    if type(x) is np.ndarray:
        assert y is not None, 'No y values provided'
        return DataLoader(TensorDataset(Tensor(x), Tensor(y).unsqueeze(1).type(torch.LongTensor), Tensor(w)), **kwargs)
    else:
        return pyg_DataLoader(x, **kwargs)
...
def get_brics(smiles_list, dataset, split='screen'):
    cache_file = f'/data2/project/junha/traversing_chem_space/data/{dataset}/{split}_brics.pkl'
    if os.path.exists(cache_file):
        with open(cache_file, "rb") as f:
            brics_cache = pickle.load(f)
        print(f"[INFO] Load cache from {cache_file}")
        return brics_cache
    ...
    for smi in tqdm(smiles_list):
        ...
        brics_cache[smi] = {'mol': frag_mols, 'smiles': frags}

    with open(cache_file, "wb") as f:
        pickle.dump(brics_cache, f)
    return brics_cache

def get_brics_fp(selected_smiles_list, brics_map, smiles_list, sim=False):
    brics_frags = set()
    for smi in selected_smiles_list:
        mol = Chem.MolFromSmiles(smi)
        frags = BRICS.BRICSDecompose(mol)
        brics_frags.update(frags)
    
    frag_list = sorted(list(brics_frags))  # feature 순서 고정
    frag_mol_list = [Chem.MolFromSmiles(f) for f in frag_list]
    frag_fp_list = [AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=1024) for mol in frag_mol_list]
    frag_index = {frag: i for i, frag in enumerate(frag_list)}
    
    fingerprints = []
    for smi in tqdm(smiles_list):
        fp = [0] * len(frag_list)
        if not sim:
            for frag in brics_map[smi]['smiles']:
                if frag in frag_index:
                    fp[frag_index[frag]] = 1
            fingerprints.append(fp)
        else:
            mol_frags_mols = brics_map[smi]['mol']
            mol_frags_fp = [AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=1024) for mol in mol_frags_mols]
            
            sim = []
            for mol_frag in mol_frags_fp:
                sim.append(DataStructs.BulkTanimotoSimilarity(mol_frag, frag_fp_list))
            fp = np.array(sim).max(axis=0)
            fingerprints.append(fp)
