...
class Handler:
    def __init__(self, n_start: int = 64, bias: str = 'random', seed: int = 42, dataset: str = 'ALDH1') -> None:
        ...
        # self.index_smiles = torch.load(os.path.join(ROOT_DIR, 'data', dataset, 'test', 'index_smiles'))
        # self.smiles_index = torch.load(os.path.join(ROOT_DIR, 'data', dataset, 'test', 'smiles_index'))
        # self.all_y = torch.load(os.path.join(ROOT_DIR, 'data', dataset, 'test', 'y'))
    ...
    def set(self, picks: Union[list, np.ndarray]):
        # Get the corresponding indices of the master dataset, and save it in self.acquired
        added_idx = np.array([self.smiles_index[smi] for smi in picks])
        self.picks = [added_idx]

        self.train_idx = added_idx
        self.screen_idx = np.array([i for i in range(len(self.all_y)) if i not in self.train_idx])
    

    def get_add(self, picks: Union[list, np.ndarray]):
        return np.array([self.smiles_index[smi] for smi in picks])
