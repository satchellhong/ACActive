...
if __name__ == '__main__':
    ...
    for feature in ['fp+mf+um']: #'fp+mf+um', 'fp+cb+um', 'fp+cb+mf', 'fp+cb+mf+um'
        ...
        #512, 0.01, 0, 4, '' / 1024, 0.01, 6, 2, '_layer2 / 256, 0.01, 0, 8, '_layer2' dim64 
        for hidn in [512]:#[256, 512, 1024]:
            for lmda in [0.01]:
                for rround in [16]:
                    args.hidden = hidn
                    for at_hidden in [4]:
                        ...
                        for layer in ['_layer2']:#_layer2
                            args.layer = layer
                            for cycle in [0]:#2, 4, 12 / 6, 8, 10
                                args.cycle = cycle
                                for dataset in ['VDR']:
                                    for beta in [0]: # , 0.5, 0.75, 1, 2
                                        ...
                                        # LOG_FILE = f"./results_0708/result_{args.dataset}_boundre_{args.arch}.csv"

                                        # os.makedirs(f"./results_0708/result_{args.dataset}_boundre_{args.arch}", exist_ok=True)
                                        # LOG_FILE = f"./results_0708/result_{args.dataset}_boundre_{args.arch}/result_{args.dataset}_boundre_{args.arch}.csv"
                                        file_name = f"77. {args.dataset}_{args.acq}_{args.arch}_{args.feature}_{args.start}_nonorm_{args.hidden}_{args.at_hidden}{args.layer}_cycle{args.cycle}_rnn2"
                                        file_name = f"81-1. {args.dataset}_{args.acq}_{args.arch}_{args.feature}_{args.start}_nonorm_{args.hidden}_{args.at_hidden}{args.layer}_cycle{args.cycle}_lambdaVar"
                                        file_name = f"87. {args.dataset}_{args.acq}_{args.arch}_{args.feature}_{args.start}_nonorm_{args.hidden}_{args.at_hidden}{args.layer}_beta{args.beta}_32T0.2"
                                        file_name = f"83. {args.dataset}_{args.acq}_{args.arch}_{args.feature}_{args.start}_nonorm_{args.hidden}_{args.at_hidden}{args.layer}_beta{args.beta}"
                                        # file_name = 'test'
                                        if args.test == 1:
                                            file_name = 'test'
                                        dir_name = f"./results_1014/83. {args.feature}_attention_kl_T1_ratio{0.25*cycle}_lambdaVar_ucb_2/{file_name}"#_weight_global_equal
                                        dir_name = f"./results_1024/87. {args.feature}_attention_cliff_{lmda}_{rround}_cliff{cycle}/{file_name}"
                                        dir_name = f"./results_1024/test_5_1/{file_name}"
                                        ...
                                        for experiment in tqdm(experiments):
                                            ...
                                            # hit_round += 1
                                            # if dataset == 'ALDH1' and results['hits_discovered'].iloc[-1] < 210:
                                            #     break
                                            # if dataset == 'PKM2' and results['hits_discovered'].iloc[-1] < 12:
                                            #     break
                                            # if dataset == 'VDR' and results['hits_discovered'].iloc[-1] < 17:
                                            #     break
                                            # if dataset == 'ALDH1' and 320*5 - hit_sum > 370*(5-hit_round):
                                            #     break
